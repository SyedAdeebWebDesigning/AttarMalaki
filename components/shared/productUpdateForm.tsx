...
import { updateProduct } from "@/lib/actions/products/products.action";
...
const ProductUpdateForm = ({ product }: ProductUpdateFormProps) => {
	...
	const form = useForm<FormValues>({
		...
		defaultValues: {
			...
			ml20: product.quantities.find((q) => q.size === "ML_20")?.price || 0,
			ml50: product.quantities.find((q) => q.size === "ML_50")?.price || 0,
			ml100: product.quantities.find((q) => q.size === "ML_100")?.price || 0,
			stock20: product.quantities.find((q) => q.size === "ML_20")?.stock || 0,
			discount20:
				product.quantities.find((q) => q.size === "ML_20")?.discountPrice ||
				undefined,
			stock50: product.quantities.find((q) => q.size === "ML_50")?.stock || 0,
			discount50:
				product.quantities.find((q) => q.size === "ML_50")?.discountPrice ||
				undefined,
			stock100: product.quantities.find((q) => q.size === "ML_100")?.stock || 0,
			discount100:
				product.quantities.find((q) => q.size === "ML_100")?.discountPrice ||
				undefined,
		},
	});
	...
	async function onSubmit(values: FormValues) {
		...
		const productData = {
			...
			].filter(Boolean), // Remove falsey sizes
		};

		try {
			await updateProduct(product.id, productData as any); // Cast to any to avoid type issues
			...
			setShowQuantities(false); // Hide quantities section
		}
	}
